...
class ProfileUpdateScreen extends StatefulWidget {
  final String? str_mobile,str_email,str_name;
  const ProfileUpdateScreen({Key? key,required this.str_mobile,required this.str_email,required this.str_name}) : super(key: key);
...
class _ProfileUpdateScreenState extends State<ProfileUpdateScreen> {
  ...
  void initState() {
    ...
    nameController.text=widget.str_name!;
    emailController.text=widget.str_email!;
    getLoginToken();
  ...
  void profileUpdate(String name, String mobilenumber, String email,) async {
    ...
    if(connection)
    {
      try {
        ...
        Response response = await dio.post(ApiService.baseUrl+"/users/update/${user_id}",
            data: {
                "name":name,
                "email":email,
        ...
        }else
        {
          ...
          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => ProfileDetailScreen()),);
                   ...
                   // height: 40,
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                          //color: Colors.orange,
                          border: Border.all(
                              color: Color(0xFF013B46), // Set border color
                              width: 2.0),   // Set border width
                          borderRadius: BorderRadius.all(
                              Radius.circular(5.0)), // Set rounded corner radius
                          //boxShadow: [BoxShadow(blurRadius: 10,color: Colors.black,offset: Offset(1,3))] // Make rounded corner of border
                      ),
                      child: Text(widget.str_mobile!,style: TextStyle(fontSize: 14),),
                    )
                    /*TextField(
                ...
                onPressed: () async {
                  ...
                  if(connection)
                  {
                    ...
                    if(nameController.text.length==0||nameController.text.isEmpty)
                    {
                      ...
                      {
                        Validation.showErrormessage(_scafkey,'Please enter  name!');
                      }else if(Platform.isIOS)
                      {
                        Validation.errorAlertDialog(context,'Please enter  name!');
                      }
                    ...
                    else if(emailController.text.length==0||emailController.text.isEmpty)
                    {
                      ...
                      if(Platform.isAndroid)
                      {
                        Validation.showErrormessage(_scafkey,'Please enter  email!');
                      }else if(Platform.isIOS)
                      {
                        Validation.errorAlertDialog(context,'Please enter  email!');
                      }
                    }else
                      {
